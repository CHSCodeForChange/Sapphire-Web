...
def addSlot(request, event_id):
		...
		if (request.method == 'GET'):
				form = NewSlotForm(user=request.user, parentEvent=parentEvent, all_members=group.volunteers)
		else:
				# This line assumes the contents of the GET side of the if statement have already run (they should have) but its kinda janky
				form = NewSlotForm(request.POST, user=request.user, parentEvent=parentEvent, all_members=group.volunteers)
				if form.is_valid():
