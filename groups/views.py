...
def console(request, group_id):
	...
	if request.user.is_authenticated():
		if group.get_is_organzer(request.user):
			...
			return render(request, 'groups/console.html',
			              {'group': group, 'user_slots': user_slots, 'feed_entries': feed_entries})
		else:
...
def update(request, group_id):
	...
	form = EditGroupForm(id=group_id,
	                     initial={'name': group.name, 'tagline': group.tagline, 'description': group.description,
	                              'email': group.email, 'website': group.website, 'location': group.location,
	                              'address': group.address, 'city': group.city,
	...
	if (group.get_is_member(request.user) == False):
		if (group.approvalNeeded):
			...
			alert = Alert(user=request.user,
			              text="Requested to join " + str(group.name) + ", wating for organizer approval",
...
def approve(request, group_id, user_id):
	...
	if (group.get_is_organzer(request.user) and group.get_is_pending(user)):
		...
		if (not (group.get_is_organzer(request.user))):
			alert = Alert(user=request.user, text="Only organizers or owners can approve pending users",
			              color=Alert.getRed())
			alert.saveIP(request)
		else:
...
def demote(request, group_id, user_id):
	group = Group.objects.get(pk=group_id)
	user = User.objects.get(pk=user_id)
	if request.user == group.owner:
		if user == group.owner:
			alert = Alert(user=request.user, text="You cannot alter the owner's permissions", color=Alert.getYellow())
			alert.saveIP(request)
			return redirect('/groups/' + str(group_id))
		if user in group.organizers.all():
			group.organizers.remove(user)
			group.volunteers.add(user)
			alert = Alert(user=request.user, text=user.username + " is now a volunteer", color=Alert.getYellow())
			alert.saveIP(request)
		...
		if user in group.volunteers.all():
			...
			alert = Alert(user=request.user, text=user.username + " is now a pending", color=Alert.getYellow())
			alert.saveIP(request)
			return redirect('/groups/' + str(group_id))
		if user in group.pendingUsers.all():
			group.pendingUsers.remove(user)
			alert = Alert(user=request.user, text=user.username + " has been removed", color=Alert.getYellow())
			alert.saveIP(request)
			return redirect('/groups/' + str(group_id))
		return redirect('/groups/' + str(group_id))
	else:
